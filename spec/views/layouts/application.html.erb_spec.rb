...
RSpec.describe 'layouts/application', type: :view do
  it 'displays the navbar correctly' do
    ...
    expect(rendered).to have_selector(
      'a.nav-link[href="#"] img.unlock-img[alt="small logo"][class="unlock-img me-2 opacity-50"] + span', text: 'Editing'
    )
    expect(rendered).to have_selector('li.nav-item.active')
    expect(rendered).to have_selector(
      'select.space.form-select.border-0.text-primary.fw-bold.fs-5[name="space"][id="space"] option.my-2[value="1"][selected]', text: 'Publish Space'
    )
